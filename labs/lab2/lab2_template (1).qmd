...
#identifying vulnerable tracts
a_vulnerable_tracts <- a_census_tract_join %>%
  mutate(
    percent = (`Total Population 65 Y+` / `Total Population`) * 100,
    a_distance = as.numeric(st_distance(st_centroid(shub$geometry), st_union(dhub))), #calculating distance between tract centroid and nearest polling places
    vulnerability = case_when(
      percent > 20 & a_distance > 804 ~ "Vulnerable!", #804 metres is 1/2 mile
      TRUE ~ "Not Vulnerable"
    )
  )
...
#creating buffer of 1/3 mile around transit stops
stop_buffer <- lhub %>%
  st_buffer(536)  


#vulnerable and critical area within tracts
critical_tracts <- a_vulnerable_tracts %>%
  st_filter(stop_buffer, .predicate = st_intersects)
...
critical_tracts %>%
  ...
  mutate(
    `County Name` = `County Name` %>%
      str_remove("; Philadelphia County")
  ) %>%
  arrange(desc(percent)) %>%
  slice_head(n = 10) %>%
  select(
    `Census Tract` = `County Name`,
    `Total Population 65 Y+`,
    `Distance to Polling Place (m)` = a_distance,
    `% Population 65+` = percent
  ) %>%
  kable(digits = 1, caption = "Critical Tracts: Vulnerable Elderly Populations with Transit Access")
