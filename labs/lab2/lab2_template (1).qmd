...
ggplot() +
  geom_sf(data = county_map_data, aes(fill = pct_underserved), color = "white") +
  scale_fill_gradient(name = "% Underserved", low = "lightyellow", high = "lightblue", na.value = "lightyellow") +
...
ggplot() +
  ...
  geom_sf(data = hos, aes(color = "Hospitals"), size = 0.5, alpha = 0.5) +
  scale_fill_gradient(name = "%", low = "lightyellow", high = "gold", na.value = "lightyellow") +
  scale_color_manual(name = "", values = c("Hospitals" = "red")) +
  guides(fill = guide_colorbar(order = 2), color = guide_legend(order = 1)) +
  labs(
    title = "Underserved Vulnerable Tracts",
    subtitle = "This choropleth only represents % of vulnerable counties that are underserved"
  ) +
  theme_void()
...
ggplot() +
  ...
  geom_sf(data = hos, aes(color = "Hospitals"), size = 0.5, alpha = 0.5) +
  scale_fill_manual(name = "Status", values = c("underserved" = "gold", "vulnerable but served" = "yellow")) +
  scale_color_manual(name = "", values = c("Hospitals" = "red")) +
  guides(fill = guide_legend(order = 2), color = guide_legend(order = 1)) +
  labs(
    title = "Underserved Vulnerable Tracts",
    subtitle = "gold tracts are vulnerable and more than 15 miles from a hospital (underserved)"
  ) +
  theme_void()
...
ggplot(vul_new) +
  aes(x = dist_near_hos) +
  geom_histogram(bins = 25, fill = "gold", color = "black", alpha = 0.7) +
